...
setwd("D:/Jiahe/columbia/mr_analysis/narnea_results/pc1_out_all_plates")
...
for(i in 1:12) {
  ...
  # Construct file path
  cnt_path <- file.path(cur_round, "narnea", paste0("drs_", cur_round, "_raw_counts.tsv"))
  md_path <- file.path(cur_round, "md", paste0("drs_", cur_round, "_txs.tsv"))
  tryCatch({
    # Read count file
    cnt_data <- read.table(cnt_path, sep="\t", header=TRUE, row.names=1, check.names=FALSE)
    
    # md file
    md_data <- read.table(md_path, sep="\t", header=TRUE, row.names=1, check.names=FALSE)
    print(md_data)
    ...
    md_rep1 <- md_data$pc1_rep1
    md_rep2 <- md_data$pc1_rep2
    md_rep3 <- md_data$pc1_rep3
